...
DSIR 不再直接按领域设置正/负方向。诊断显示原始 DSIR 与 `word_count` 存在严重混杂，因此先在 A1 各来源域内依据 `log1p(word_count)` 的分位数建立长度层，再在同长度层内计算正向经验百分位：

\[
u_{ij}^{\mathrm{DSIR}}
=
\widehat F_{d,b(L_i),j}(x_{ij}).
\]

长度分箱与条件参考分布只由 A1 拟合，A2/A3 只复用，不重新估计。该处理将 DSIR 组与长度结构组的 pooled Spearman 从约 0.899 降到 -0.0852，同时保持与语义质量组约 0.219 的正相关。
